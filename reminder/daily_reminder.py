...
async def daily_reminder_func(bot_id:int) -> None:
    ...
    try:
        list_user = await ServiceFactory.get_booking_service(bot_id).get_inactive_users_missing_future_bookings()
        await ServiceFactory.get_notification_service(bot_id).send_message_to_users(list_user, REMINDER_TEXT, reply_markup=get_user_start_buttons())
